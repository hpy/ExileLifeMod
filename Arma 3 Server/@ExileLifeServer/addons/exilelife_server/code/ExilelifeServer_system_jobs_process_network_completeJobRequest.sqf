/**
                    * ExilelifeServer_system_jobs_process_network_completeJobRequest
                    *
                    * Exile Mod
                    * www.exilemod.com
                    * © 2016 Exile Mod Team
...
try
{
	...
	{
		_code = missionNamespace getVariable [format["ExileLifeServer_system_jobs_scenes_%1_onCompletion",_filePath],""];
	...
	switch (_jobType) do {
	    case 1: {
			...
			if !(isNull _offeredByPlayer)then{
				...
				_offeredByPlayerMsg = getText(configFile >> "CfgJobs" >> _jobClassname >> "offeredByPlayerOnCompletedMsg");
		...
		{
			{
				...
				if  !(isNull _pObject) then
				{
					...
					if (_recipientSessionID call ExileServer_system_session_isRegisteredId)then{
						[_recipientSessionID,"completeJobTask",[_jobClassname,str(_pObject getVariable ["ExileScore",0])]] call ExileServer_system_network_send_to;
					};
					if (isText(configFile >> "CfgJobs" >> _jobClassname >> "givenVehicle")) then {
			...
			if (_sessionID call ExileServer_system_session_isRegisteredId)then{
				[_sessionID,"completeJobTask",[_jobClassname,str(_playerObject getVariable ["ExileScore",0])]] call ExileServer_system_network_send_to;
			};
			if !((typename _completedJobs) == "ARRAY") then
			{
				_completedJobs = [];
