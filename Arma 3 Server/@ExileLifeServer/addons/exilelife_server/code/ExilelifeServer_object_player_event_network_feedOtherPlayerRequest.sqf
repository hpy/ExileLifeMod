/**
                    * ExilelifeServer_object_player_event_network_feedOtherPlayerRequest
                    *
                    * Exile Mod
                    * www.exilemod.com
                    * © 2016 Exile Mod Team
...
try
{
	...
	if !(isClass _config) then
	{
		...
		throw format["%1 is not properly configured to be a food item!", _item];
	};
	if !(getNumber(_config >> "isFoodItem") isEqualTo 1) then 
	{
		_requestingPlayer setVariable ["ExileLifeForcedConsuming", ""];
		_target setVariable ["ExileLifeForcedConsuming", ""];
		throw format["%1 is not properly configured to be a food item!", _item];
