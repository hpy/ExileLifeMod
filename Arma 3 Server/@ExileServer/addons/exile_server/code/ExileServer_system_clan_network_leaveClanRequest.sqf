/**
                    ...
                    */
                    ...
                    private["_sessionID","_player","_clanID","_clanHash","_playerUID","_isLeader","_leaderUID","_clanMembers","_clanMembersCount","_index","_leaderPID","_message"];
...
try
{
	...
	if(_clanMembersCount > 1)then
	{
		...
		if(_isLeader)then
		{
			...
			_leaderPID = ((_leaderUID call ExileLifeServer_system_session_getIDFromUID) call ExileServer_system_session_getPlayerObject) getVariable ["ExileLifePID",-1];
			_clanHash set [1,_leaderUID];
			_clanHash set [6,_leaderPID];
			format["updateClanLeader:%1:%2",_leaderUID,_leaderPID,_clanID] call ExileServer_system_database_query_fireAndForget;
