/**
                    ...
                    */
                    ...
                    private["_payload","_sessionId","_messageName","_messageParameters","_playerBySessionId","_sessionID","_ExileSessionID","_config","_allowedParameters","_moduleName","_addonName","_functionName","_functionCode"];
_payload = _this;
try 
{
	...
	if !((_messageParameters select 0) in [19120]) then
	{
		format["MessageName: %1 | MessageParameters: %2  |  AllowedParameters: %3",_messageName,_messageParameters,_allowedParameters] call ExileLifeSErver_util_debugLog;
	};
	...
	switch (_addonName) do 
	{
		...
		case "exilelife":
		{
			_functionName = format ["ExileLifeServer_%1_network_%2", _moduleName, _messageName];
		};
	};
