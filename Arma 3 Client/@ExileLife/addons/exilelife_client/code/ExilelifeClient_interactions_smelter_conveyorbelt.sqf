/**
                    * ExilelifeClient_interactions_smelter_conveyorbelt
                    *
                    * ExileLife Mod
                    ...
                    */
                    ...
                    private["_interactionname","_zone","_number","_alreadyrunning","_parameters","_hasACan","_magazines","_response","_soundobject","_timedeletearray","_soundsource","_list","_object","_interactionmsg","_emptypositionclassname","_emptycontainerpos","_emptycontainers","_emptycontainer","_emptycontainernetId","_fillpositionclassname","_fillcontainerpos","_fillcontainers","_fillcontainer","_fillcontainernetId"];
...
_list = position player nearObjects ["Land_HelipadEmpty_F",7];
{
...
_emptypositionclassname = getText(missionconfigFile >> "CfgExileLifeInteraction" >> _zone >> _interactionname >> "emptypositionclassname");
_emptycontainerpos = getArray(missionconfigFile >> "CfgExileLifeInteraction" >> _zone >> _interactionname >> "emptycontainerpos");
_emptycontainers = _emptycontainerpos nearObjects [_emptypositionclassname,7];
_emptycontainer = _emptycontainers select 0;
_emptycontainernetId = netId _emptycontainer;
_fillpositionclassname = getText(missionconfigFile >> "CfgExileLifeInteraction" >> _zone >> _interactionname >> "fillpositionclassname");
_fillcontainerpos = getArray(missionconfigFile >> "CfgExileLifeInteraction" >> _zone >> _interactionname >> "fillcontainerpos");
_fillcontainers = _fillcontainerpos nearObjects [_fillpositionclassname,7];
_fillcontainer = _fillcontainers select 0;
_fillcontainernetId = netId _fillcontainer;
["startConveyorBeltRequest",[_emptycontainernetId,_fillcontainernetId]] call ExileClient_system_network_send; 
}
else
{
	if !(_alreadyrunning) then 
	{
		["ErrorTitleAndText", ["No Fuel!", "You need fuel to start the conveyorbelt!"]] call ExileClient_gui_toaster_addTemplateToast;
		_response = false;
