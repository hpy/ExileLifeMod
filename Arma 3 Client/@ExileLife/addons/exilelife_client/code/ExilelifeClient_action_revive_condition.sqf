/**
                    * ExilelifeClient_action_revive_condition
                    *
                    * ExileLife Mod
...
try
{
	...
	{
		throw "You are not listed as an available medic!";
	};
	...
	if !(isNull _medic)then{
		if(_medic != player)then{
			throw "Trying to revive someone twice are we?";
		};
	}else{
	...
	_victim setVariable ['ExileLifeClientReviveState', 2,true];
}
catch
{
	_result = _exception;
